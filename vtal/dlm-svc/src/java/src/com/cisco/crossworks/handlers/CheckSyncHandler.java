...
public class CheckSyncHandler implements Handler {
    private static final Logger log = Logger.getLogger(CheckSyncHandler.class);

    @Override
    public Object handle(NsoMessage msg, MaapiSession ms) throws IOException, ConfException {
        log.info("Device Check Sync...");
        ...
        for (NsoDeviceData nsoDeviceData : nsoDeviceList.getDeviceListList()) {
            ...
            if (device == null) {
                ...
                try {
                    ConfXMLParam[] opr = ms.getMaapi().requestAction(new ConfXMLParam[0], "/ncs:devices/device{%s}/check-sync", device.getName());
        ...
        log.info("Check Sync was executed on " + nsoDeviceList.getDeviceListList().size() + " devices.");
        return result.build();
    }
