...
public class SyncFromHandler implements Handler {
    private static final Logger log = Logger.getLogger(SyncFromHandler.class);

    @Override
    public Object handle(NsoMessage msg, MaapiSession ms) throws IOException, ConfException {
        log.info("Device Sync From...");
        ...
        for (NsoDeviceData nsoDeviceData : nsoDeviceList.getDeviceListList()) {
            ...
            if (device == null) {
                ...
                try {
                    log.info("Executing Sync-From for device: " + device.getName());
                    ConfXMLParam[] opr = ms.getMaapi().requestAction(new ConfXMLParam[0], "/ncs:devices/device{%s}/sync-from", device.getName());
                    ...
                    log.info("Sync-From has failed for device " + device.getName() + " with the following exception:", e);
                    HandlerUtils.setDeviceSucess(nsoDeviceData, result, false, HandlerUtils.getErrorCause(e));
                }
        }
        log.info("Sync From was executed on " + nsoDeviceList.getDeviceListList().size() + " devices.");
        return result.build();
    }
