...
class IosXR:
    def get_interface_loopback(self, root, service):
        ...
        for loopback in device.config.interface.Loopback:
            if loopback.ipv4.address.ip == device_address:
                return loopback.id

        return None

    def conf_rsvp_te_tunnel_p2p(self, service, loopback):
        self.log.info(f"Configuring RSVP TE Tunnel on IOSXR device {service.head_end}")

        #  Applying connfig
        vars = ncs.template.Variables()
        vars.add("LOOPBACK", loopback)
        template = ncs.template.Template(service)
        template.apply("cisco-rsvp-te-fp-cli-template", vars)

    def ietf_te_self_test(self, uinfo, root, rsvp_te_service):
        device = rsvp_te_service.head_end
        self.log.info(f"Running IETF-TE self test on IOSXR device {device}")
        ...
        if is_netsim_device(self, root, device):
            ...
            return self.run_self_test(action, action_input, tunnel_name, device, rsvp_te_service)
