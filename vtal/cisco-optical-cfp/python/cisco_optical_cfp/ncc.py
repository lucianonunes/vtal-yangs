...
NCSN_1_0_NS = "urn:ietf:params:xml:ns:yang:ietf-subscribed-notifications"

NMDA_NS = "urn:ietf:params:xml:ns:yang:ietf-netconf-nmda"
NMDA_DS_NS = "urn:ietf:params:xml:ns:yang:ietf-datastores"
NMDA_OR_NS = "urn:ietf:params:xml:ns:yang:ietf-origin"
...
class NetconfSSH(NetconfSSHLikeTransport):
    ...
    def _start_ssh(self, sock):
        ...
        if self.publicKeyType == "rsa":
            agent_public_key = paramiko.RSAKey(data=base64.decodestring(self.publicKey))
        elif self.publicKeyType == "dss":
            agent_public_key = paramiko.DSSKey(data=base64.decodestring(self.publicKey))
        else:
...
def create_connection(host, port):
    ...
    for res in socket.getaddrinfo(host, port, socket.AF_UNSPEC, socket.SOCK_STREAM):
        ...
        break
    if sock is None:
        raise StdoutError("Failed to connect to {0}: {1}".format(host, lasterr))
    return sock
...
def get_msg(cmd, db, o):
    ...
    xpath = o.xpath
    subtree_filter = o.subtree_filter
    ...
    if cmd == "get-config":
        fstr = mk_filter_str(xpath, subtree_filter)
        ...
        if subtree_filter:
            ...
            dataf = get_file(subtree_filter)
            subtree_txt = dataf.read().decode("utf-8")
            dataf.close()
            st_filter = subtree_template.format(subtree_txt)
        config_false = ""
        ...
        or_ns_xmlns_decl = ""
        if o.with_origin or o.origin_filter or o.negated_origin_filter:
            or_ns_xmlns_decl = ' xmlns:or="' + NMDA_OR_NS + '"'
        with_origin = ""
        if o.with_origin:
            with_origin = "<with-origin/>"
        origin_filter = ""
        if o.origin_filter:
            origin_filter = "".join(
                [
                    "<origin-filter%s>or:%s</origin-filter>" % (or_ns_xmlns_decl, x)
                    for x in o.origin_filter
                ]
            )
        negated_origin_filter = ""
        if o.negated_origin_filter:
            negated_origin_filter = "".join(
                [
                    "<negated-origin-filter%s>"
                    "or:%s"
                    "</negated-origin-filter>" % (or_ns_xmlns_decl, x)
                    for x in o.negated_origin_filter
                ]
            )
        op1 = """<get-data xmlns="{0}" xmlns:ds="{1}"{2}>
                 <datastore>ds:{3}</datastore>
...
def create_subscription_msg(stream, xpath, subtree_filter, start, stop):
    fstr = mk_filter_str(xpath, subtree_filter)
...
<rpc xmlns="{0}" message-id="1">
  <create-subscription xmlns="urn:ietf:params:xml:ns:netconf:notification:1.0">
    ...
    {4}
        ...
        NC_NS, stream, fstr, startstr, stopstr
    )


def establish_subscription_msg(stream, xpath, subtree_filter, start, stop):
    if xpath:
        fstr = mk_stream_filter_str("stream-xpath-filter", xpath)
    elif subtree_filter:
        data = get_file(subtree_filter).read().decode("utf-8")
        fstr = mk_stream_filter_str("stream-subtree-filter", data)
    else:
        fstr = ""
    ...
    if not start:
        ...
        startstr = "<replay-start-time>{0}+00:00</replay-start-time>".format(start)

    if not stop:
        stopstr = ""
    else:
        stopstr = "<stop-time>{0}+00:00</stop-time>".format(stop)

    return """<?xml version="1.0" encoding="UTF-8"?>
<rpc xmlns="{0}" message-id="1">
  <establish-subscription xmlns=\"{1}\">
    {2}
    <stream>{3}</stream>
    {4}
    {5}
        ...
        NC_NS, NCSN_1_0_NS, fstr, stream, startstr, stopstr
    )


def kill_subscription_msg(subscription_id):
    return """<?xml version="1.0" encoding="UTF-8"?>
<rpc xmlns="{0}" message-id="1">
  <kill-subscription xmlns=\"{1}\">
    <id>{2}</id>
  </kill-subscription>
</rpc>""".format(
        NC_NS, NCSN_1_0_NS, subscription_id
    )


def mk_filter_str(xpath, subtree_filter):
    if xpath:
        if "'" in xpath:
        ...
        data = get_file(subtree_filter).read().decode("utf-8")
        return "<filter type='subtree'>{}</filter>".format(data)
    else:
        return ""


def mk_stream_filter_str(filter_field, filter_content):
    if "'" in filter_content:
        return '<{0} xmlns="{1}">{2}</{0}>'.format(
            filter_field, NCSN_1_0_NS, filter_content
        )
    else:
        return '<{0} xmlns="{1}">{2}</{0}>'.format(
            filter_field, NCSN_1_0_NS, filter_content
...
def read_msg():
    ...
    print(
        ...
        ", or only empty continuing session"
    )

    ln = sys.stdin.readline()
    if (ln == "\n") or (ln == ""):
        # send empty message and continue if we're expecting meta/control
        # notification event to the transport session further on.
        return None

    msg = """<?xml version="1.0" encoding="UTF-8"?>
    <rpc xmlns="{0}" message-id="{1}">
    """.format(
        NC_NS, read_msg.msgid
    )
...
def build_options_parser():
    ...
    parser.add_option(
        "-v", "--version", dest="version", help="force NETCONF version 1.0 or 1.1"
    )
    parser.add_option("-d", "--debug", dest="debug", action="store_true")
    parser.add_option("-u", "--user", dest="username", default="admin", help="username")
    parser.add_option(
        "-p", "--password", dest="password", default="admin", help="password"
    )
    parser.add_option(
        "--proto",
        dest="proto",
    ...
    parser.add_option(
        "--device", dest="device", default=None, help="NETCONF device in NSO"
    )
    parser.add_option(
        "--host", dest="host", default="localhost", help="NETCONF server hostname"
    )
    parser.add_option(
        "--port", dest="port", default=2022, type="int", help="NETCONF server port"
    )
    parser.add_option(
        "--listen",
        dest="listen",
    ...
    parser.add_option(
        ...
        default=1,
        type="int",
        help="Sends the same request ITER times.  Useful only in" " test scripts",
    )
    ...
    styleopts.add_option(
        "-s",
        "--outputStyle",
        dest="style",
        default="default",
        help="Display output in: raw, plain, pretty, all, " "or noaaa format",
    )
    ...
    cmdopts.add_option(
        ...
        " or --subtree-filter for subtree filtering.",
    )
    cmdopts.add_option(
        "--get-config",
        action="callback",
        ...
        "filtering or --subtree-filter for subtree filtering.",
    )
    cmdopts.add_option(
        "--get-data",
        action="callback",
        ...
        "an optional --max-depth argument, "
        "an optional --origin-filter argument or"
        "an optional --negated-origin-filter argument.",
    )
    cmdopts.add_option(
        "--max-depth",
        dest="max_depth",
        default="",
        help="Can be used to limit the depth of the " "--get-data command.",
    ),
    cmdopts.add_option(
        "--config-filter",
        dest="config_filter",
    ...
    cmdopts.add_option(
        ...
        dest="subtree_filter",
        default="",
        help="Takes a filename (or '-' for standard input or '-- string --' from string) as "
    ...
    cmdopts.add_option(
        ...
        dest="with_origin",
        action="store_true",
        help="If present, the 'origin' annotation for " "the nodes will be returned.",
    )
    cmdopts.add_option(
        "--origin-filter",
        dest="origin_filter",
        action="append",
        default=[],
        help="It can be used with --get-data. "
        "If one or more origin filter is set, only nodes "
        "derived from given origin will be returned.",
    )
    cmdopts.add_option(
        "--negated-origin-filter",
        dest="negated_origin_filter",
        action="append",
        default=[],
        help="It can be used with "
        "--get-data. If one or more origin filter is set, "
        "only nodes that are not derived from given origin "
        "will be returned.",
    )
    cmdopts.add_option(
        "--db",
    ...
    cmdopts.add_option(
        ...
        "--get-data, --create-subscription and "
        "--establish-subscription",
    )
    cmdopts.add_option(
        "--kill-session",
    ...
    cmdopts.add_option("--discard-changes", dest="discard_changes", action="store_true")
    ...
    cmdopts.add_option(
        "--copy-running-to-startup", dest="copy_running_to_startup", action="store_true"
    )
    cmdopts.add_option(
        "--copy-config",
        dest="copy",
    ...
    cmdopts.add_option(
        "--create-subscription",
        dest="create_subscription",
        help="Takes a stream name as parameter, and an optional "
        "-x for XPath filtering or --subtree-filter"
        " for subtree filtering.",
    )
    cmdopts.add_option(
        "--establish-subscription",
    ...
    cmdopts.add_option(
        "--kill-subscription",
        dest="kill_subscription",
        type="int",
        help="Takes a subscription id as parameter",
    )
    cmdopts.add_option(
        "--replay-start-time",
        dest="start_time",
    ...
    cmdopts.add_option(
        ...
        help="Takes a timestamp in YYYY-MM-DDTHH:MM:SS format, "
        "for e.g. 2017-01-10T12:05:21. Assumes UTC. Used with "
        "--create-subscription or --establish-subscription."
        "The session will be closed after stop-time has"
        "been reached",
    )
    cmdopts.add_option(
        "--rpc-attribute",
...
def validate_options(o):
    if o.wdefaults and o.wdefaults not in (
        "trim",
        "explicit",
        "report-all",
        "report-all-tagged",
    ...
    OF = o.origin_filter
    NOF = o.negated_origin_filter
    if OF or NOF:
        valid = ["default", "dynamic", "intended", "learned", "system", "unknown"]
        if any(f not in valid for f in OF + NOF):
            msg = (
                "Origin must be one of {0}.\n"
                "Use --rpc to use your own origin identity."
            )
            arg = " | ".join(["'%s'" % origin for origin in valid])
            raise StdoutError(msg.format(arg))
...
def main(parser, o):
    ...
    if filename:
        ...
        msg = create_subscription_msg(
            o.create_subscription, o.xpath, o.subtree_filter, o.start_time, o.stop_time
        )
    elif o.establish_subscription is not None:
        msg = establish_subscription_msg(
            o.establish_subscription,
            o.xpath,
            o.subtree_filter,
            o.start_time,
            o.stop_time,
        )
    elif o.kill_subscription is not None:
        msg = kill_subscription_msg(o.kill_subscription)
    elif o.hello:
        msg = None
    elif o.interactive:
        pass
    ...
    if o.proto == "ssh":
        c = NetconfSSH(
            o.host, o.port, o.username, o.password, "", "", o.privKeyFile, o.privKeyType
        )
    else:
    ...
    if d is not None:
        strip(d)
        if (
            d.namespaceURI == NC_NS
            and d.localName == "hello"
            and d.firstChild is not None
        ):
            d = d.firstChild
            strip(d)
            if d.namespaceURI == NC_NS and d.localName == "capabilities":
                d = d.firstChild
                strip(d)
                while d is not None:
                    if d.namespaceURI == NC_NS and d.localName == "capability":
                        if (
                            "1.1" in versions
                            and d.firstChild.nodeValue.strip() == BASE_1_1
                        ):
    ...
    if cmdf is not None or dataf is not None:
        ...
        if cmdf is not None:
            ...
            send(
                """<?xml version="1.0" encoding="UTF-8"?>
                    <rpc xmlns="{0}" message-id="1">""".format(
                    NC_NS
                )
            )
        if o.edit:
            ...
            send(
                "<edit-config xmlns:nc='{0}'>"
                "<target><{1}/></target><config>".format(NC_NS, o.db)
            )
        elif o.edit_data:
            # the file contains the edit-data payload only; send rpc
            send(
                "<edit-data xmlns='{0}' xmlns:ds='{1}'>"
                "<datastore>ds:{2}</datastore><config>".format(
                    NMDA_NS, NMDA_DS_NS, o.db
                )
            )
        elif o.copy:
            # the file contains the copy-config payload only; send rpc
            send("<copy-config><target><{0}/></target><source><config>".format(o.db))
        ...
        n = o.iter
        if (
            o.create_subscription is not None
            or o.interactive
            or o.establish_subscription is not None
        ):
            if o.style == "default":
                o.style = "all"
            # print results until EOF
    ...
    if o.style == "noaaa":
        noaaa = (
            "|awk ' BEGIN { del=0} /<(aaa|nacm)/ {del=1} /<\\/(aaa|nacm)>/"
    ...
    if (o.create_subscription is not None or o.establish_subscription is not None) and (
        o.stop_time != ""
    ):
        stop_time = calendar.timegm(time.strptime(o.stop_time, "%Y-%m-%dT%H:%M:%S"))
    else:
        stop_time = 0
    ...
    while (
        not is_closed
        and (n != 0 or o.interactive)
        and not do_get_schema
        and ((stop_time == 0) or (replay_complete == 0) or (time.time() < stop_time))
    ):
        fd = None
        if do_print and o.style in ("pretty", "all", "noaaa"):
            p = subprocess.Popen(
                xmllint, shell=True, stdin=subprocess.PIPE, stderr=subprocess.PIPE
            )
            fd = p.stdin
            p.stderr.close()
        else:
...
if __name__ == "__main__":
    ...
    try:
        if o.device is not None:
            # Get device info from NSO
            with ncs.maapi.single_read_trans("", "system") as th:
                root = ncs.maagic.get_root(th)
                device = root.devices.device[o.device]
                device_authgroup_name = device.authgroup
                authgroup = root.devices.authgroups.group[device_authgroup_name]
                ## Check for umap first
                if o.username in authgroup.umap:
                    umap = authgroup.umap[o.username]
                    o.username = umap.remote_name
                    epassword = umap.remote_password
                else:
                    o.username = authgroup.default_map.remote_name
                    epassword = authgroup.default_map.remote_password
                th.maapi.install_crypto_keys()
                o.password = _ncs.decrypt(epassword)
                o.host = device.address
                o.port = device.port
