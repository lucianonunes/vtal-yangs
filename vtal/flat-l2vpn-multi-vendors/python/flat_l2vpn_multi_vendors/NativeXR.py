...
class NativeXR:
    def conf_l2vpn(self, site, local):
        self.log.info(f"Configuring Flat L2VPN/SR on IOSXR NETCONF for service {site.pe}")
        mappingv4 = "\n"
        mappingv6 = "\n"
        mappingevpn_rt = "\n"
        mappingevpn_mp = "\n"
        ...
        template = "cisco-flat-L2vpn-fp-native-p2p-template"
        if self.service.service_type == "evpn-vpws":
            template = "cisco-flat-L2vpn-fp-native-evpn-vpws-template"
        elif self.service.service_type == "evpn-multipoint":
            template = "cisco-flat-L2vpn-fp-native-evpn-multipoint-template"
    ...
    def conf_l2vpn_rp(self, rr_parent_route_policy):
        ...
        rp_template.apply("cisco-flat-L2vpn-fp-route-policy-iosxr-nc", rp_vars)
    ...
    def validate_parent_policy_exists(self, root, device, parent_policy):
        ...
        if parent_policy in device.config.routing_policy.route_policies.route_policy:
            return True
        return False

    def get_original_policy(self, root, device, parent_policy):
        # Return route policy value
        return (root.devices.device[device].config.routing_policy
                .route_policies.route_policy[parent_policy].rpl_route_policy)
    ...
    def check_if_interface_exists(self, root, site,
        ...
        admin_status = root.devices.device[site.pe].\
            live_status.interfaces.interface_briefs.interface_brief[service_interface].state
    ...
    def get_interface_shutdown_template(self):
        return "cisco-flat-L2vpn-fp-no-shutdown-iosxr-nc"
