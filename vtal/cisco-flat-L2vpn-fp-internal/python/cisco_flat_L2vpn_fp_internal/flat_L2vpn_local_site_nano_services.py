...
class FlatL2vpnValidator(object):
    ...
    def cb_validate(self, tctx, kp, newval):
        TsdnUtils.validate_service(self, Utils.local_site_validation_callpoint, tctx, kp)
    ...
    def cb_pre_modification(self, tctx, op, kp, root, proplist):
        ...
        try:
            ...
            if (op == _ncs.dp.NCS_SERVICE_CREATE) or (op == _ncs.dp.NCS_SERVICE_UPDATE):
                ...
                self.log.info("Pre-Mod for local-site service is redeploy:"
                              + f"{is_redeploy}")
                self.log.info(f"L2VPN local-site pre_mod internal Opaque: {opaque}")
                ...
                if service.flat_L2vpn_evpn_vpws:
                    ...
                    parent_rr_route_policy = service.\
                        flat_L2vpn_evpn_vpws.local_site.sr_te.odn.\
                        attach_point.parent_rr_route_policy

                    if parent_rr_route_policy:
                        # Get device impl class
                        device = service.flat_L2vpn_evpn_vpws.local_site.pe
                ...
                if not (is_redeploy) and l2vpn_validation_enabled:
                    ...
                    if (root.devices.device[service.pe].address != localhost
                        ...
                        try:
                            if not router.check_if_interface_exists(root, site, str(site.if_type),
                                ...
                                raise UserErrorException(
                                    ...
                                    "Provided interface not present on local site",
                                ).add_state(
    ...
    def _create_ready(self, _tctx, root, service, plan,
        ...
        if "true" == opaque.get("DEVICE_TRANS_INVALIDATED"):
            self.log.info(f"Device Transaction for service {service.name} "
                          f"device {service.pe} is invalidated, marking ready failed")
