...
class NativeXR:
    def get_interface_loopback(self, root, service):
        ...
        for (act_loopback) in (
            device.
            config.Cisco_IOS_XR_ifmgr_cfg__interface_configurations.interface_configuration
        ):
            if "Loopback" in act_loopback.interface_name:
                loopback = act_loopback.interface_name.replace("Loopback", "")
                address = act_loopback.\
                    Cisco_IOS_XR_ipv4_io_cfg__ipv4_network.addresses.primary.address
    ...
    def conf_rsvp_te_tunnel_p2p(self, service, loopback):
        ...
        template.apply("cisco-rsvp-te-fp-native-template", vars)
