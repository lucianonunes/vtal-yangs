...
class UMXR:
    ...
    def template_sr_policy_template_new(service, device):
        ...
        sr_policy_template.apply("cisco-sr-te-cfp-um-sr-odn-policy", sr_policy_vars)

    def conf_sr_template(self, device):
        self.log.info(f"Configuring SR ODN policy template on IOSXR NC device {device}")
        UMXR.template_sr_policy_template_new(self.service, device)
    ...
    def template_sr_segment_list(service, device, name, index, label, ipv4):
        ...
        sr_segment_list_template.apply("cisco-sr-te-cfp-um-sr-segment-list",
    ...
    def template_sr_policy(service, endpoint, device):
        ...
        sr_policy_vars.add("HEAD_END", device)
        sr_policy_template = ncs.template.Template(service)
        sr_policy_template.apply("cisco-sr-te-cfp-um-sr-non-odn-policy",
                                 sr_policy_vars)
    ...
    def conf_sr_policy(self, device):
        ...
        UMXR.template_sr_policy(service=self.service, endpoint=endpoint, device=device)
    ...
    def get_max_sid_depth(self, root, device):
        ...
        return (root.devices.device[device].config.
                Cisco_IOS_XR_um_segment_routing_cfg__segment_routing.
                Cisco_IOS_XR_um_segment_routing_traffic_eng_cfg__traffic_eng.maximum_sid_depth)
    ...
    def run_self_test(self, uinfo, root, service, device):
        ...
        for retry in range(max_retries):
            try:
                ...
                (admin_result, oper_result) = UMXR.get_self_test_result(uinfo, device, policy_name)
